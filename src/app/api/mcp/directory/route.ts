...
function parseAwesomeMcpMarkdown(markdown: string): PublicMcpServer[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("- [") && line.includes("](http")) {
      ...
      // Only match genuine endpoint URLs (excluding GitHub, Glama, mcpservers.org, etc.)
      const httpEndpointMatch = line.match(
        /(https?:\/\/(?!github\.com|glama\.ai|mcpservers\.org|mcp\.so|twitter\.com|x\.com|medium\.com|youtube\.com)[^\s\)`"']+(?:\/mcp|\/sse)[^\s\)`"']*)/i
      );
      ...
      if (httpEndpointMatch) {
        ...
        command = language === "python" ? `uvx ${pkgName.toLowerCase()}` : `npx -y ${pkgName.toLowerCase()}`;
      }
      ...
      // Check if command has placeholders like <PATH_TO_VAULT> or <DATABASE_URL>
      const hasPlaceholders = Boolean(command && /<[A-Z0-9_]+>/i.test(command));

      // Determine requires auth from description or placeholders
      const requiresAuth = hasPlaceholders ||
...
export async function GET(req: Request) {
  ...
  if (smitheryResult.status === "fulfilled") {
    ...
    smitheryServers = rawServers.map((s: Record<string, unknown>) => {
      ...
      const baseEndpoint = s.deploymentUrl ? String(s.deploymentUrl) : `https://server.smithery.ai/@${s.qualifiedName}/sse`;
      const endpointUrl = isRemote
        ? (smitheryKey && !baseEndpoint.includes("api_key=") ? `${baseEndpoint}${baseEndpoint.includes("?") ? "&" : "?"}api_key=${smitheryKey}` : baseEndpoint)
        : undefined;

      // Detect auth requirement: remote Smithery servers require an API key, or description mentions auth
      const requiresAuth =
        isRemote ||
        description.toLowerCase().includes("api key") ||
        description.toLowerCase().includes("token") ||
      ...
      return {
        ...
        description,
        owner: namespace,
        stars: typeof s.useCount === "number" ? s.useCount : 0,
        tags,
        transport,
