            ...
            const streamIterable = await llm.stream([
              {
                role: "system",
                content:
                  "You are the Agent Hub A2A Autonomous Orchestrator responding to an inbound task delegation under the Google A2A Protocol. Provide a structured, high-quality response.",
              },
    ...
    const completion = await llm.complete([
      {
        role: "system",
        content:
          "You are the Agent Hub A2A Autonomous Orchestrator. Process this delegated task cleanly and return a structured synthesis.",
      },
      { role: "user", content: `TASK INPUT:\n${promptText}` },
    ]);
