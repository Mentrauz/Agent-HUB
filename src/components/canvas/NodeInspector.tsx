...
export function NodeInspector({ node, onUpdate, onDelete, allNodeIds, onOpenSubgraph, onToggleBreakpoint }: NodeInspectorProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {type === "code_sandbox" && (
        <>
          <Field label="Programming Language">
            <select
              value={node.data.codeLanguage ?? "python"}
              onChange={(e) => onUpdate({ codeLanguage: e.target.value as "javascript" | "python" })}
              className={`${inputClass} cursor-pointer`}
            >
              <option value="python">Python (Safe MicroVM)</option>
              <option value="javascript">JavaScript (V8 Sandbox)</option>
            </select>
          </Field>
          <Field
            label="Executable Script / Function"
            hint="Access input via 'input', state via 'results', and return the final payload"
          >
            <textarea
              value={node.data.codeSnippet ?? ""}
              onChange={(e) => onUpdate({ codeSnippet: e.target.value })}
              rows={8}
              spellCheck={false}
              placeholder={
                node.data.codeLanguage === "python"
                  ? "# Python sandbox script\nitems = input.get('items', [])\ntotal = sum(items)\nreturn {'total': total, 'count': len(items)}"
                  : "// JavaScript sandbox\nconst items = input.items || [];\nconst total = items.reduce((a, b) => a + b, 0);\nreturn { total, count: items.length };"
              }
              className={`${inputClass} resize-y text-[9px] font-mono leading-relaxed`}
            />
          </Field>
        </>
      )}

      {/* ─── Swarm Consensus Inspector ─── */}
      {type === "consensus" && (
        <>
          <Field label="Consensus Strategy">
            <select
              value={node.data.consensusStrategy ?? "majority_vote"}
              onChange={(e) =>
                onUpdate({
                  consensusStrategy: e.target.value as
                    | "majority_vote"
                    | "judge_synthesis"
                    | "highest_confidence",
                })
              ...
              className={`${inputClass} cursor-pointer`}
            >
              <option value="majority_vote">Majority Vote / Primary Model</option>
              <option value="judge_synthesis">Referee Judge LLM Synthesis</option>
              <option value="highest_confidence">Highest Confidence Response</option>
            </select>
          </Field>
          <Field
            label="Candidate Swarm Models (JSON Array)"
            hint="Models polled concurrently during consensus election"
          >
            <textarea
              value={
                node.data.consensusModels
                  ? JSON.stringify(node.data.consensusModels, null, 2)
                  : '[\n  "groq/compound",\n  "google/gemma-4-26b-a4b-it:free",\n  "cohere/north-mini-code:free"\n]'
              }
              onChange={(e) => {
                try {
                  onUpdate({ consensusModels: JSON.parse(e.target.value) });
                } catch {}
              }}
              rows={4}
              className={`${inputClass} resize-y text-[9px] font-mono leading-relaxed`}
      ...
      {/* ─── Red-Team Critic Loop Inspector ─── */}
      {type === "critic" && (
        <>
          <Field label="Evaluation Rubric & Constraints" hint="Strict rules the critic validates against">
            <textarea
              value={node.data.criticRubric ?? ""}
              onChange={(e) => onUpdate({ criticRubric: e.target.value })}
              rows={4}
              placeholder="1. Verify factual accuracy against references\n2. Flag any compliance or security risks\n3. Ensure response is clear, structured, and under 500 words"
              className={`${inputClass} resize-y text-[9px] leading-relaxed`}
            />
          </Field>
          <div className="grid grid-cols-2 gap-2">
            <Field label="Pass Score (0-100)">
              <input
                type="number"
                min={50}
                max={100}
                value={node.data.criticPassScore ?? 80}
                onChange={(e) => onUpdate({ criticPassScore: Number(e.target.value) })}
                className={inputClass}
              />
            </Field>
            <Field label="Max Refine Turns">
              <input
                type="number"
                min={1}
                max={5}
                value={node.data.criticMaxTurns ?? 2}
                onChange={(e) => onUpdate({ criticMaxTurns: Number(e.target.value) })}
                className={inputClass}
              />
            </Field>
          </div>
