...
/** Interactive Form field schema for HITL nodes. */
export interface ApprovalFormField {
  name: string;
  label: string;
  type: "text" | "number" | "select" | "multiselect" | "boolean" | "date" | "textarea" | "json";
  required?: boolean;
  options?: string[];
  placeholder?: string;
...
export interface GraphNodeData {
  ...
  /** Interactive Form Schema fields to collect from the human reviewer. */
  ...
  // code_sandbox
  /** Target programming language for the sandboxed code node. */
  codeLanguage?: "javascript" | "python";
  /** Script / function body to execute inside the sandbox. */
  codeSnippet?: string;
  /** Variable / template bindings injected into the code scope. */
  codeInputs?: Record<string, unknown>;
  /** Execution timeout in milliseconds (defaults to 5000ms). */
  codeTimeoutMs?: number;
  // consensus
  /** Models to poll in parallel for swarm consensus. */
  consensusModels?: string[];
  /** Aggregation strategy for consensus voting. */
  consensusStrategy?: "majority_vote" | "judge_synthesis" | "highest_confidence";
  // critic
  /** Optional target template or content to critique (defaults to node.data.prompt or upstream output). */
  criticTarget?: string;
  /** Rubric / guardrails for red-team critique. */
  criticRubric?: string;
  /** Max refinement cycles. */
  criticMaxTurns?: number;
  /** Minimum score (0-100) required to pass review. */
  criticPassScore?: number;
  // memory
  /** Whether episodic & semantic memory is enabled for this node. */
  enableMemory?: boolean;
  /** Memory tags to filter retrieved memories. */
  memoryTags?: string[];
  // breakpoint
