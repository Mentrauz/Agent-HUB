...
/**
 * Bidirectional MCP support: Agent Hub acts as an MCP *server* so external
  ...
  private buildSkillInputSchema(version: SkillVersionDTO): Record<string, unknown> {
    ...
    return {
      ...
      properties: {
        userId: {
          type: "string",
          description: "Agent Hub user id whose account runs this workflow. Required for every invocation.",
        },
        ...properties,
      },
    ...
    try {
      ...
      const text = JSON.stringify({
        ...
        ...(execution.status === "PAUSED_FOR_APPROVAL"
          ? { approvalRequired: true, message: "Execution paused — approve or reject it in the Agent Hub review queue" }
          : {}),
