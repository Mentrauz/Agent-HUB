 ...
 * completions back from Agent Hub's engine (nested agentic behavior).
 *
...
export class McpConnection implements McpRpcClient {
  ...
  private async connectWith(
    ...
    // request to Agent Hub, which can proxy it to its configured LLM.
  ...
  private buildStdioTransport(): StdioClientTransport {
    ...
    if (typeof process !== "undefined" && !process.env.AWS_LAMBDA_FUNCTION_NAME && !process.env.VERCEL) {
      ...
      throw new Error(
        ...
        `or self-host Agent Hub on a persistent server (e.g. a VPS or Docker).`
      );
    }
    ...
    if (typeof process !== "undefined" && process.platform === "win32") {
      ...
      if (
        (lower === "npx" || lower === "npm" || lower === "pnpm" || lower === "yarn" || lower === "uvx" || lower === "pipx") &&
        !lower.endsWith(".cmd") &&
        !lower.endsWith(".exe") &&
        !lower.endsWith(".bat")
      ) {
      ...
      for (const [key, value] of Object.entries(this.server.headers)) {
        if (typeof value === "string" && value.trim()) {
          ...
          if (key.toLowerCase() === "authorization") {
            ...
            // Map provider-specific tokens if detected from command or server name
            const hint = `${this.server.name} ${commandLine}`.toLowerCase();
            if (hint.includes("github")) {
              env["GITHUB_PERSONAL_ACCESS_TOKEN"] = token;
              env["GITHUB_TOKEN"] = token;
            } else if (hint.includes("notion")) {
              env["NOTION_API_KEY"] = token;
              env["NOTION_API_TOKEN"] = token;
            } else if (hint.includes("brave")) {
              env["BRAVE_API_KEY"] = token;
              env["BRAVE_SEARCH_API_KEY"] = token;
            } else if (hint.includes("slack")) {
              env["SLACK_BOT_TOKEN"] = token;
            } else if (hint.includes("stripe")) {
              env["STRIPE_SECRET_KEY"] = token;
            } else if (hint.includes("supabase")) {
              env["SUPABASE_API_KEY"] = token;
            } else if (hint.includes("linear")) {
              env["LINEAR_API_KEY"] = token;
            } else if (hint.includes("sentry")) {
              env["SENTRY_AUTH_TOKEN"] = token;
            } else if (hint.includes("qdrant")) {
              env["QDRANT_API_KEY"] = token;
            }
          }
        }
      }
  ...
  private buildStreamableTransport(): StreamableHTTPClientTransport {
    ...
    const headers = this.getOutboundHeaders();
  ...
  private getOutboundHeaders(): Record<string, string> | undefined {
    const headers: Record<string, string> = { ...(this.server.headers ?? {}) };
    
    // Auto-inject Smithery API key for hosted Smithery servers if configured in environment
    if (
      typeof process !== "undefined" &&
      process.env?.SMITHERY_API_KEY &&
      this.server.endpointUrl?.includes("smithery.ai") &&
