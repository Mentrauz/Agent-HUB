...
export function mcpserversOrgToServer(entry: McpServersOrgEntry): {
  ...
  if (/python|py\b|uvx/i.test(slug)) language = "python";
  else if(/\bgo\b|golang/i.test(slug)) language = "go";
  else if(/rust|rs\b|cargo/i.test(slug)) language = "rust";
  else if(/ruby/i.test(slug)) language = "ruby";
  else if(/java\b/i.test(slug)) language = "java";
  else if(/ts|typescript|js|node/i.test(slug)) language = "typescript";

  const isRemote = entry.source === "remote";
  const isSkill = entry.source === "skills";
  const isPriority = entry.source === "priority";

  // Derive transport: mcpservers.org remote entries are remote-hosted, but their sitemap
  // URLs are HTML webpages. Only set transport to SSE if we have a real SSE stream URL.
  const transport: "STDIO" | "SSE" = isRemote ? "SSE" : "STDIO";

  // Build the repo URL
  const repoUrl = `https://github.com/${slug}`;

  // IMPORTANT: entry.url is an HTML web page (e.g. https://mcpservers.org/remote-mcp-servers/...)
  // and MUST NOT be used as the MCP endpointUrl, otherwise the SSE/HTTP client receives HTML
  // and crashes JSON-RPC parsing.
  const endpointUrl = undefined;

  // For STDIO servers, derive language-aware command
  let command: string | undefined = undefined;
  if (!isRemote) {
    if (language === "python") {
      command = `uvx ${repoName}`;
    } else {
      command = `npx -y ${repoName}`;
    }
  }

  // Detect auth requirement from slug patterns
  // Any unverified external sitemap server should require config review to avoid blind broken spawns
  const requiresAuth = isRemote || /api[-_]?key|token|auth|oauth|secret|password/i.test(slug) || !isPriority;
  ...
  return {
    ...
    name: `${cleanName} MCP`,
    description: `MCP server from mcpservers.org (${isRemote ? "Remote server" : isSkill ? "Agent skill" : "STDIO server"}). Source: ${entry.url}`,
    owner,
    repoUrl,
    transport,
    command,
