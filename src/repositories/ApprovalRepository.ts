...
export class ApprovalRepository implements IApprovalRepository {
  ...
  async respond(input: RespondApprovalInput): Promise<ApprovalRequestDTO | null> {
    // If formData provided, fetch current payload to merge
    let updatedPayload: Record<string, unknown> | undefined = undefined;
    if (input.formData && input.approved) {
      const current = await prisma.approvalRequest.findUnique({
        where: { id: input.approvalId },
        select: { inputPayload: true },
      });
      if (current && typeof current.inputPayload === "object" && current.inputPayload !== null) {
        updatedPayload = {
          ...(current.inputPayload as Record<string, unknown>),
    ...
    const result = await prisma.approvalRequest.updateMany({
      ...
      data: {
        ...
        ...(updatedPayload ? { inputPayload: updatedPayload as unknown as Prisma.InputJsonValue } : {}),
      },
    });
