...
[![Tests](https://img.shields.io/badge/Tests-83_Suites_630_Passing-brightgreen?style=for-the-badge&logo=vitest&logoColor=white)](https://vitest.dev)
...
- **14 Distinct Node Types**:
  - `start` / `end`: Input entry and final payload emission terminals.
  - `agent`: Autonomous LLM node with role-specific system prompts, tool allowances, and episodic/semantic memory retrieval.
  ...
  - `approval`: Human-in-the-Loop gate with optional interactive form fields, auto-approval conditions, and escalation timeouts.
  ...
  - `code_sandbox`: Isolated V8 JavaScript or Python sandbox for deterministic data transformations, filtering, and math without host exposure.
  - `consensus`: Dynamic multi-model swarm polling with majority vote, highest confidence, or judge synthesis.
  - `critic`: Iterative red-team adversarial critique, scoring (0-100), and refinement loop against custom evaluation rubrics.
  - `sticky_note`: Canvas annotation and documentation callout with markdown formatting and color themes.
...
- **Agent Hub as an MCP Server**: External agents (Cursor, Claude Desktop, Antigravity) connect to `/api/mcp/sse` (Streamable HTTP + SSE) and access published workflows as callable tools. Auth via Clerk session or `MCP_ACCESS_TOKEN` bearer token.
...
### 13. Event-Driven Webhooks & Scheduled Cron Triggers (`/services/TriggerService.ts` & `/api/triggers`)
- **Inbound Webhook System**: Generate cryptographically secure tokens (`wh_...`) tied to published skills or drafts.
- **HMAC-SHA256 Signature Verification**: Supports GitHub, Stripe, and custom signature headers (`x-hub-signature-256`, `sha256=...`) using configurable secrets with constant-time comparison.
- **Dynamic Payload Mapping**: Map nested JSON paths from `{ body, headers, query }` directly into graph input variables using syntax like `{{body.customer.id}}` or `{{headers.x-request-id}}`.
- **Scheduled Cron Jobs**: Configure standard 5-part cron expressions with customizable payload templates and automated execution dispatching.
- **Trigger Analytics & Control**: Track trigger execution count, last executed timestamps, and toggle active/paused states via REST.

### 14. Interactive Form Human-in-the-Loop (HITL) (`/modules/approval/` & `/dashboard/review`)
- **Form Schema Configuration**: Define dynamic review forms with field types: `text`, `number`, `select`, `multiselect`, `boolean`, `date`, `textarea`, and `json`.
- **Interactive Review UI**: Reviewers fill out required parameters, adjust values, and provide structured input during approval.
- **Downstream State Injection**: Submitted form responses are automatically injected into execution state at `ctx.results[approvalNodeId]`, making user input immediately accessible to downstream nodes via `{{ results.<approvalNodeId>.field }}`.
- **Idempotent Single-Use Submissions**: Cryptographic atomicity guarantees form responses cannot be submitted more than once or replayed.

### 15. Sandboxed Code Execution Node (V8 JS & Safe Python) (`/modules/execution/sandbox/`)
- **Isolated V8 JavaScript VM**: Execute deterministic data wrangling, calculations, array filtering, and string transformations with standard Math, JSON, Date, RegExp, and Array methods.
- **Safe Python Runtime**: Supports Python data structures, comprehensions, and built-in functions (`len()`, `sum()`, `min()`, `max()`, `str()`, `int()`, `float()`, `bool()`, `print()`).
- **Zero Host Exposure**: Runs in a locked execution context without access to sensitive Node.js APIs (`process`, `require`, `fs`, network).
- **Configurable Timeouts**: Set per-node timeouts via `codeTimeoutMs` (default 5000ms) with stdout log capture and duration metrics.

### 16. Persistent Cross-Session Agent Memory (`/services/MemoryService.ts`)
- **Episodic & Semantic Knowledge Store**: Store and retrieve user preferences, facts, and conversation context across multiple execution sessions.
- **Scoped & Global Memories**: Associate memories with specific skills or globally with the user across all agent interactions.
- **Automatic System Prompt Injection**: When `enableMemory` is enabled on an `agent` node, relevant memories matching query or tags are formatted and prepended to the system prompt.
- **Recency & Relevance Tracking**: Automatically tracks `lastAccessedAt` and supports tag-based retrieval filtering.

### 17. Multi-Agent Swarm Consensus & Adversarial Critic Nodes
- **Consensus Node (`consensus`)**: Dispatches identical prompts in parallel across multiple models (e.g. Groq, Gemma, Cohere) and aggregates conclusion using `majority_vote`, `highest_confidence`, or an impartial `judge_synthesis` LLM.
- **Red-Team Critic Node (`critic`)**: Runs iterative critique -> scoring (0-100) -> refinement loops against strict user rubrics until the score threshold is met or `maxTurns` is reached.
- **Custom Target Templates**: Supports targeted critique on specific upstream outputs via `criticTarget` or `prompt` (e.g. `{{ results.draft_writer.text }}`).

### 18. 1-Click Eject to Standalone Code (LangGraph Python & TypeScript) (`/components/canvas/exportUtils.ts`)
- **Zero Vendor Lock-In**: Export visual canvas graphs directly to standalone, runnable Python and TypeScript code.
- **LangGraph Python Export**: Generates complete `langgraph` Python scripts utilizing `StateGraph(AgentState)`, typed state dictionaries, async node functions, and entry point routing.
- **LangGraph.js TypeScript Export**: Generates modern `@langchain/langgraph` code with `Annotation.Root()` state schemas and `.compile()` runtime graphs.
- **Full Node Coverage**: Exports all nodes including `agent`, `supervisor`, `tool`, `router`, `approval`, `loop`, `parallel`, `code_sandbox`, `consensus`, and `critic` with environment variable fallbacks (`AGENT_STUDIO_API_URL`).
...
graph TB
    ...
    subgraph Server["Agent Hub MCP Server"]
    ...
    subgraph ClientSide["Agent Hub MCP Client"]
...
| `GET / POST` | `/api/triggers` | List or create webhook and cron triggers for skills |
| `DELETE` | `/api/triggers?id=:id&type=:type` | Delete a webhook trigger or scheduled cron |
| `GET / POST` | `/api/triggers/webhook/:token` | Public inbound webhook receiver endpoint (HMAC verified) |
| `POST` | `/api/triggers/cron/:id` | Execute a scheduled cron trigger |
...
- **83 Test Suites Passed (630 / 630 Tests Passing)** with 100% success rate.
- **0 ESLint Errors & 0 Warnings** across all components, API routes, and modules.
- **0 TypeScript Type Errors** under strict compiler configuration (`npx tsc --noEmit`).
- **Clean Next.js 15 Production Build** across all pages and 75+ dynamic API endpoints.
